...
- 默认使用中文与用户沟通，包括阶段总结、计划说明、风险提示和完成报告。
- 生成或修改面向本仓库协作流程的文档时，默认使用中文；若文档需要遵循既有英文格式、代码 API 约定或用户明确要求英文，则保持对应语言。
- 代码标识符、命令、错误类型、测试名称和公共 API 名称保持项目原有英文风格。
...
- 支持两个或多个命名表之间的显式 `INNER JOIN ... ON ...` 等值连接。
- 支持 `INT`、`FLOAT`、`TEXT`、`BOOL` 类型系统。
- 支持 B-tree 索引、单文件磁盘持久化、ACID 事务和 CLI/REPL。
- 支持同一数据库文件的跨线程、跨进程保守单写者并发安全；冲突在配置的超时后以 `ConcurrencyError` 返回。
